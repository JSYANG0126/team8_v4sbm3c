...
window.onload = function() {
  ...
  function checkID() {
    ...
    if (id.value.trim().length == 0) {
      ...
      $.ajax({
        url: './checkID.do', // spring execute, http://localhost:9091/mem/checkID.do?id=user1@gmail.com
        type: 'get',  // post
      ...
      // 처리중 출력
      id_msg.innerHTML="<img src='/mem/images/ani04.gif' style='width: 6%;'>"; // static 기준
...
<c:import url="/menu/top.do" />
  ...
  <FORM name='frm' id='frm' method='POST' action='./update.do' class="">
    ...
    <div class="form-group"> <%-- 줄이 변경되지 않는 패턴 --%>
      ...
      <button type='button' id="btn_checkID" onclick="checkID()" class="btn btn-sm" style="background-color: #323232; color: white;">중복확인</button>
      <span id='id_msg'></span>
    </div>   
  
    <div class="form-group"> <%-- label의 크기에따라 input 태그의 크기가 지정되는 형태 --%>
      <label>성명*:
        <input type='text' class="form-control form-control-sm" name='mname' id='mname' value='${memVO.mname }' required="required" placeholder="성명">
      </label>
    ...
    <div class="form-group"> <%-- label의 크기에따라 input 태그의 크기가 지정되는 형태, 줄이 변경되지 않는 패턴 --%>
      <label>전화 번호:
        <input type='text' class="form-control form-control-sm" name='tel' id='tel' value='${memVO.tel }' required="required" placeholder="전화번호">
      </label>
      예) 010-0000-0000
    </div>   
    ...
    <div class="form-group">
      ...
      <button type="button" id="btn_DaumPostcode" onclick="DaumPostcode()" class="btn btn-sm" style="background-color: #323232; color: white;">우편번호 찾기</button>
    </div>  

    <div class="form-group">
      <label style="width: 100%;">주소:</label> <%-- label의 크기를 변경하여 주소를 많이 입력받는 패턴 --%>
      <input type='text' class="form-control form-control-sm" name='address1' id='address1' value='${memVO.address1 }' placeholder="주소">
    </div>   

    <div class="form-group">
      <label style="width: 100%;">상세 주소:</label>
      <input type='text' class="form-control form-control-sm" name='address2' id='address2' value='${memVO.address2 }' placeholder="상세 주소">
    </div>   
...
<!-- ------------------------------ DAUM 우편번호 API 종료 ------------------------------ -->
    ...
    <div class="form_input">
      <button type="button" id='btn_send' onclick="send()" class="btn btn-sm" style="background-color: #323232; color: white;">저장</button>
