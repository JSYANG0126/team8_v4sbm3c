...
public class MovieProc implements MovieProcInter {
  ...
  public ArrayList<MovieVO> list_by_genreno(int genreno) {
	ArrayList<MovieVO> list = this.movieDAO.list_by_genreno(genreno);
	return list;
  }
  ...
  @Override
	...
	public int map(HashMap<String, Object> map) {
		int cnt = this.movieDAO.map(map);
		return cnt;
	}
	
	@Override
	public int youtube(HashMap<String, Object> youtube) {
		int cnt = this.movieDAO.youtube(youtube);
		return cnt;
	}

	@Override
	public ArrayList<MovieVO> list_by_genreno_search(HashMap<String, Object> hashMap) {
		ArrayList<MovieVO> list = this.movieDAO.list_by_genreno_search(hashMap);
		return list;
	}

	@Override
	public int search_count(HashMap<String, Object> hashMap) {
		int cnt = this.movieDAO.search_count(hashMap);
		return cnt;
	}
	...
	@Override
	  public ArrayList<MovieVO> list_by_genreno_search_paging(MovieVO movieVO) {
	    ...
	    */
	    int begin_of_page = (movieVO.getNow_page() - 1) * Movie.RECORD_PER_PAGE;
	    ...
	    */
	    ...
	    movieVO.setStart_num(start_num);
	    movieVO.setEnd_num(end_num);
	    
	    ArrayList<MovieVO> list = this.movieDAO.list_by_genreno_search_paging(movieVO);
...
//	    text-align: center;    
	...
	//  
	//  <DIV id='paging'>
//	    <span class='span_box_1'><A href='list_by_genreno.do?word=&now_page=1&cateno=4'>1</A></span>
	//  <span class='span_box_2'>2</span>
	//  <span class='span_box_1'><A href='list_by_genreno.do?word=&now_page=3&genreno=4'>3</A></span>
//	    </DIV>
	//  </DIV> 
	//  <!-- 페이지 목록 출력 부분 종료 -->
...
//	      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	      ...
	      if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
	        str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&genreno="+genreno+"'>이전</A></span>"); 
	      } 
	      ...
	      for(int i=start_page; i<=end_page; i++){ 
	        ...
	        if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
	          ...
	          str.append("<span class='span_box_1'><A href='"+list_file+"?word="+word+"&now_page="+i+"&genreno="+genreno+"'>"+i+"</A></span>");   
	        } 
	      } 
	      ...
	      if (now_grp < total_grp){ 
	        str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&genreno="+genreno+"'>다음</A></span>"); 
	      } 
	      str.append("</DIV>"); 
	    ...
	    public int password_check(HashMap<String, Object> hashMap) {
	      int cnt = this.movieDAO.password_check(hashMap);
	      return cnt;
	    }
		...
		public int delete(int movieno) {
			int cnt = this.movieDAO.delete(movieno);
		    return cnt;
		}

		@Override
		public int count_by_genreno(int genreno) {
			int cnt = this.movieDAO.count_by_genreno(genreno);
		      return cnt;
		}
