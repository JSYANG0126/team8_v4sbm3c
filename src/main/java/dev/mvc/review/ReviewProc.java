...
public class ReviewProc implements ReviewProcInter {
  ...
  public ArrayList<ReviewVO> list_search_paging(ReviewVO reviewVO) {
    ...
    */
    int begin_of_page = (reviewVO.getNow_page() - 1) * Review.RECORD_PER_PAGE;
    ...
    */
    ...
    ArrayList<ReviewVO> list = this.reviewDAO.list_search_paging(reviewVO);
  ...
  public String pagingBox(int now_page, String word, String list_file, int search_count){
    ...
    int total_page = (int)(Math.ceil((double)search_count / Review.RECORD_PER_PAGE));
    // 전체 그룹  수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹-> 2 그룹
    int total_grp = (int)(Math.ceil((double)total_page / Review.PAGE_PER_BLOCK)); 
    // 현재 그룹 번호: (double)13/10 -> 1.3 -> 2 그룹
    int now_grp = (int)(Math.ceil((double)now_page / Review.PAGE_PER_BLOCK));  
    ...
    str.append("    border-color: #323232;"); 
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public ReviewVO read(int reviewno) {
    ReviewVO read = this.reviewDAO.read(reviewno);
    return read;
  }
